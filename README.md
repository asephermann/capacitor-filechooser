...
constructor(private platfrom: Platform) {
    ...
    //   this.chooseFile();
      ...
      // mime type .txt or .jpg 
    //   this.chooseFile(["text/plain", "image/jpeg"]);
...
chooseFile = async (mime_types?: Array<string>) => {
    const result = await FileChooser.open({mime_types});
...
| Param        | Type                                    |
| ------------ | --------------------------------------- |
| **`filter`** | <code>{ mime_types?: string[]; }</code> |
